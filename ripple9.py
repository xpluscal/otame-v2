import opc
import time
import math
import random

# Configuration
MATRIX_SIZE = 8
TOTAL_LEDS_MATRIX = MATRIX_SIZE * MATRIX_SIZE
TOTAL_LEDS_STRIP = 64
TOTAL_LEDS = TOTAL_LEDS_MATRIX + TOTAL_LEDS_STRIP
NOISE_FACTOR = 0.2
RIPPLE_SPEED = 0.10
SPEED_VARIANCE = 0.05  # This will add variance to the speed of each iteration.

client = opc.Client('localhost:7890')  # Initialize the Fadecandy client

# Precompute random frequencies and phase offsets for each LED
frequencies = [random.uniform(0.05, 0.15) for _ in range(TOTAL_LEDS)]
phases = [random.uniform(0, 2 * math.pi) for _ in range(TOTAL_LEDS)]

def hue_to_rgb(hue):
    r = int(255 * (1 - hue))
    b = int(255 * hue)
    return r, 0, b

def lerp(start, end, fraction):
    return start + fraction * (end - start)

def ripple_value(dist, t):
    """Calculate ripple value based on distance and time."""
    frequency = 2.0  # adjust this for the number of ripples you want
    scaled_distance = dist * 2.0  # you can adjust the scaling factor to stretch or compress the ripple effect
    
    # Calculate the base sine value for the ripple.
    sine_val = math.sin(frequency * (scaled_distance - t * (1 + random.uniform(-SPEED_VARIANCE, SPEED_VARIANCE))))
    
    # Create a tighter window function to produce a clearer ring.
    window = max(0, 1 - 10 * abs(sine_val))
    
    return window * (1 + sine_val) / 2

def main():
    t = 0
    CENTER_X, CENTER_Y = MATRIX_SIZE // 2, MATRIX_SIZE // 2
    TARGET_X, TARGET_Y = random.randint(0, MATRIX_SIZE-1), random.randint(0, MATRIX_SIZE-1)

    try:
        while True:
            if t == 0:
                TARGET_X, TARGET_Y = random.randint(0, MATRIX_SIZE-1), random.randint(0, MATRIX_SIZE-1)

            pixels = []

            # Calculate the current center point
            current_center_x = lerp(CENTER_X, TARGET_X, t / (2 * math.pi))
            current_center_y = lerp(CENTER_Y, TARGET_Y, t / (2 * math.pi))

            # Calculate pixels for both matrices
            for matrix_index in range(2):  # One for the actual matrix and one for the strip
                for i in range(MATRIX_SIZE):
                    for j in range(MATRIX_SIZE):
                        dist = math.sqrt((current_center_x - i)**2 + (current_center_y - j)**2)
                        brightness = ripple_value(dist, t)

                        base_hue = (math.sin(time.time() * 0.05) + 1) / 2  # Base hue oscillates between 0 and 1

                        # Oscillate the noise around the base hue
                        led_index = matrix_index * TOTAL_LEDS_MATRIX + i * MATRIX_SIZE + j
                        noise = NOISE_FACTOR * math.sin(time.time() * frequencies[led_index] + phases[led_index])
                        hue = base_hue + noise
                        hue = max(0, min(hue, 1))  # Ensure hue remains within [0, 1]

                        r, g, b = hue_to_rgb(hue)
                        pixels.append((int(r * brightness), int(g * brightness), int(b * brightness)))

            client.put_pixels(pixels)
            time.sleep(0.05)
            t += RIPPLE_SPEED
            if t > 2 * math.pi:  # Complete one ripple cycle
                t = 0
                CENTER_X, CENTER_Y = TARGET_X, TARGET_Y  # Set the current center to the target for the next iteration
    except KeyboardInterrupt:
        pixels = [(0, 0, 0)] * TOTAL_LEDS
        client.put_pixels(pixels)  # Turn off LEDs when stopping the script

if __name__ == "__main__":
    main()
